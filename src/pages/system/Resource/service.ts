 ...
 * @return {Promise}
 */
export const queryMenuDetail = (id: number): Promise<IResponse> => {
    return axiosInstance.post('/api/system/menu/queryMenuDetail', {id}).then(res => res.data);
};
...
export const queryMenuList = (params: MenuListParam): Promise<IResponse> => {
    return axiosInstance.post('/api/system/menu/queryMenuResourceList',params).then(res => res.data);
};
